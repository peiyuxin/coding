package org.pyx.apache.poi.excel.helper.cache;
...
import org.pyx.apache.poi.excel.helper.util.ClassAssistant;
...
public class CacheFactory {
    ...
    private static final ConcurrentHashMap<Class<?>, int[]> COLUMN_WIDTH = new ConcurrentHashMap<>();
    ...
    public static int[] findColumnWidth(Class<?> cls) {
        int[] columnWidth = COLUMN_WIDTH.get(cls);
     ...
     * @return
     */
    public static Map<Integer, HeaderInfo> findHeaderInfo(Class<?> cls) {
        Map<Integer, HeaderInfo> headerInfo = HEADER_INFO.get(cls);
        if (MapUtils.isEmpty(headerInfo)) {
            headerInfo = ClassAssistant.getHeaderInfo(cls);
            HEADER_INFO.put(cls, headerInfo);
        }
        return headerInfo;
    }

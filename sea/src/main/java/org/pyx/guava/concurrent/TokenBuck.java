 ...
 * 令牌桶算法：必须读写分离的情况下，限制写的速率或者饥饿营销的场景  只卖1分种抢购1000
 *
...
public class TokenBuck {
    ...
    public int buy(){
        ...
        //原因当saleCount=99 时 同时存在三个线程进来。虽然phoneNumbers原子性，但是也会发生。如果必须写在这里，在success
        //里面也需要加上double check
       /* if (saleCount.get()>=saleLimit){
        ...
        if (success){
            if (saleCount.get() >= saleLimit){
                throw new IllegalStateException("Cargo has been sale "+saleLimit+" can not buy more...");
            }
            int no = saleCount.getAndIncrement();
            System.out.println(currentThread()+" user has get :["+no+"]");
            return no;
        }else {
